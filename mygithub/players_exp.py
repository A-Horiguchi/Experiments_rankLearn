...
if __name__ == '__main__':
    ...
    parser.add_argument('-del', '--delta', type=int, help='面積推定時の座標分割数', default=1000)
        ...
        for rb in listrb:
            ...
            for np in listp:
                ...
                if not args.dimtest_actual:
                    ...
                    ## 検証
                    if os.path.exists(result_dir+'/'+str(rp)+'_'+str(rb)+'/cor.csv') and os.path.exists(result_dir+'/'+str(rp)+'_'+str(rb)+'/trurh_area.csv') and os.path.exists(result_dir+'/'+str(rp)+'_'+str(rb)+'/result_area.csv'):
                        ### csv open
